...
formula01 <- ExhaustionLevel ~ Performance.DMSMean + Performance.AEDMean + Performance.DDCMean
rnafatig <- neuralnet(formula01, treino, hidden = c(4,2), lifesign = "full", linear.output = FALSE, threshold = 0.01, algorithm = 'sag')#, learningrate = 0.3)

plot(rnafatig, rep="best")

teste.01 <- subset(teste, select = c("Performance.DMSMean", "Performance.AEDMean", "Performance.DDCMean"))
